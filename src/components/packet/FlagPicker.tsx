...
export default function FlagPicker({ value, onChange }: { value: string; onChange: (flagId: string) => void }) {
  ...
  const items: FlagItem[] = FLAG_GROUPS.flatMap(g => g.items)
  ...
  const filteredGroups = FLAG_GROUPS.map(g => ({
    ...g,
      ...
      {open && (
        ...
        >
          ...
          <div className="overflow-auto tp-scroll" style={{ maxHeight: '130px' }}>
            ...
            {filteredGroups.length === 0 && <div style={{ fontSize: '7px', color: '#64748b', padding: '6px', textAlign: 'center' }}>No flags found</div>}
          </div>
        </div>
      )}
    </div>
